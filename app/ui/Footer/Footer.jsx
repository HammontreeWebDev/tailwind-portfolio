...
export default function Footer() {
  return (
    <footer className="relative border-t border-canvas-border bg-canvas-elevated" role="contentinfo">
      <div className="mx-auto max-w-7xl overflow-hidden px-6 py-16 sm:py-20 lg:px-8">
        <div className="flex flex-col items-center gap-8">
          ...
          <nav aria-label="Footer" className="flex flex-wrap justify-center gap-x-8 gap-y-3">
            {navigation.main.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-sm text-ink-subtle transition-colors hover:text-secondary"
              >
                {item.name}
              </a>
            ))}
          ...
          <div className="flex gap-6" role="list" aria-label="Social media">
            {navigation.social.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-ink-subtle transition-colors hover:text-secondary"
                target="_blank"
          ...
          <div className="flex gap-6">
            <a href={bootCampBadgeData.url} target="_blank" rel="noopener noreferrer" aria-label="UCF Coding Boot Camp certification">
              <Image
                alt="UCF Coding Boot Camp badge"
                height={72}
                width={72}
                loading="lazy"
            ...
            <a href={cyberBadgeData.url} target="_blank" rel="noopener noreferrer" aria-label="UCF Cyber Defense certification">
              <Image
                alt="UCF Cyber Defense Professional badge"
                height={72}
                width={72}
                loading="lazy"
          ...
          <p className="text-center text-xs text-slate">
            &copy; {new Date().getFullYear()} Hammontree Full-Stack Solutions LLC. All rights reserved.
          </p>
        </div>
      </div>
    </footer>
